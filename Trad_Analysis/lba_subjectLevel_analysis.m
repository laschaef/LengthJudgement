...
% for fi = [1, 2, 6, 8, 9, 10, 12, 13, 16, 18, 19, 22, 26, 28, 29, 30, 32, 34, 35, 36, 37, 38, 40, 42, 43, 44, 45, 48, 50, 52]
    ...
    % remove all trials were previous trial was incorrect - so keep all
    % trials were previous trial was correct
%     real_data(real_data(:,13)=='1',:) = [];
    
    % remove all trials were previous trial was correct - so keep all
    % trials were previous trial was incorrect (should be smaller # of
    % trials)
%     real_data(real_data(:,13)=='0',:) = [];
...
% for i = [1, 2, 6, 8, 9, 10, 12, 13, 16, 18, 19, 22, 26, 28, 29, 30, 32, 34, 35, 36, 37, 38, 40, 42, 43, 44, 45, 48, 50, 52]
    if i == 14 || i == 24 || i == 20 || i == 49 || i == 51 || i == 53 % have overall accuracy < 70% (determined in R)
        opt_params_summary(i,:)=repelem(NaN,23);
