...
% for fi = [1, 2, 6, 8, 9, 10, 12, 13, 16, 18, 19, 22, 26, 28, 29, 30, 32, 34, 35, 36, 37, 38, 40, 42, 43, 44, 45, 48, 50, 52]
    ...
    for i = 1:length(real_data)
        if real_data(i,5) == 'NO AID' && real_data(i,12) == '1'
            real_data(i,end) = 1;  % No Aid, Previous Incorrect
        elseif real_data(i,5) == 'NO AID' && real_data(i,12) == '0'
            real_data(i,end) = 2;   % No Aid, Previous Correct
        elseif real_data(i,4)=='low' && real_data(i,12) == '1'
            real_data(i,end) = 3;   % High accuracy, Previous Incorrect
        elseif real_data(i,4)=='low' && real_data(i,12) == '0'
            real_data(i,end) = 4;   % High accuracy, Previous Correct
        elseif real_data(i,4)=='high' && real_data(i,12) == '1'
            real_data(i,end) = 5;   % Low accuracy, Previous Incorrect
        elseif real_data(i,4)=='high' && real_data(i,12) == '0'
            real_data(i,end) = 6;   % Low accuracy, Previous Correct
        end 
    end
    
%     need to fix excel for these still (7/4/2019)
%     for i = 1:length(real_data)
%         if real_data(i,5) == 'NO AID' && real_data(i,12) == '1' && real_data(i,3) == 'easy'
%             real_data(i,end) = 1;  % No Aid, Easy, Previous Incorrect
%         elseif real_data(i,5) == 'NO AID' && real_data(i,12) == '1' && real_data(i,3) == 'hard'
%             real_data(i,end) = 2;  % No Aid, Hard, Previous Incorrect
%         elseif real_data(i,5) == 'NO AID' && real_data(i,12) == '0' && real_data(i,3) == 'easy'
%             real_data(i,end) = 3;   % No Aid, Easy, Previous Correct
%         elseif real_data(i,5) == 'NO AID' && real_data(i,12) == '0' && real_data(i,3) == 'hard'
%             real_data(i,end) = 4;   % No Aid, Hard, Previous Correct
%         elseif real_data(i,4)=='low' && real_data(i,12) == '1' && real_data(i,3) == 'easy'
%             real_data(i,end) = 5;   % High accuracy, Easy, Previous Incorrect
%         elseif real_data(i,4)=='low' && real_data(i,12) == '1' && real_data(i,3) == 'hard'
%             real_data(i,end) = 6;   % High accuracy, Hard, Previous Incorrect
%         elseif real_data(i,4)=='low' && real_data(i,12) == '0' && real_data(i,3) == 'easy'
%             real_data(i,end) = 7;   % High accuracy, Easy, Previous Correct
%         elseif real_data(i,4)=='low' && real_data(i,12) == '0' && real_data(i,3) == 'hard'
%             real_data(i,end) = 8;   % High accuracy, Hard, Previous Correct
%         elseif real_data(i,4)=='high' && real_data(i,12) == '1' && real_data(i,3) == 'easy'
%             real_data(i,end) = 9;   % Lowi accuracy, Easy, Previous Incorrect
%         elseif real_data(i,4)=='high' && real_data(i,12) == '1' && real_data(i,3) == 'hard'
%             real_data(i,end) = 10;   % Low accuracy, Hard, Previous Incorrect
%         elseif real_data(i,4)=='high' && real_data(i,12) == '0' && real_data(i,3) == 'easy'
%             real_data(i,end) = 11;   % Low accuracy, Easy, Previous Correct
%         elseif real_data(i,4)=='high' && real_data(i,12) == '0' && real_data(i,3) == 'hard'
%             real_data(i,end) = 12;   % Low accuracy, Hard, Previous Correct
%         end 
%     end    
    ...
    data.cond = str2double(real_data(2:end,13));  % condition
...
%     nReps = 150;  % 200 took a very long time to run
    nReps = 200;
    ...
    %     models = 3;
        ...
        model(2).v = 12; model(2).A = 1; model(2).b = 1; model(2).t0 = 1; model(2).sv = 1;
        model(3).v = 1; model(3).A = 1; model(3).b = 12; model(3).t0 = 1; model(3).sv = 1;
        model(4).v = 12; model(4).A = 1; model(4).b = 12; model(4).t0 = 1; model(4).sv = 1; 
    %     model(1).v = 10; model(1).A = 1; model(1).b = 10; model(1).t0 = 1; model(1).sv = 1; 
%         v = 2*rand.*[.6 .7 .6 .7 .7 .8];
%         b = 2*rand.*[200 200 200 200 150 150];
        v = 2*rand.*[.6 .7 .6 .7 .6 .7 .6 .7 .7 .8 .7 .8];
        b = 2*rand.*[200 200 200 200 200 200 200 200 150 150 150 150];
    ...
    %     pArray{1} = [v rand*400 b rand*0.2 rand*1];
        ...
        names{1} = ['v \t A \t b \t sv \t t0'];
        names{2} = ['v1 \t v2 \t v3 \t v4 \t v5 \t v6 \t v7 \t v8 \t v9 \t v10 \t v11 \t v12 \t A \t b \t sv \t t0'];
        names{3} = ['v \t A \t b1 \t b2 \t b3 \t b4 \t b5 \t b6 \t b7 \t b8 \t b9 \t b10 \t b11 \t b12 \t sv \t t0'];
        names{4} = ['v1 \t v2 \t v3 \t v4 \t v5 \t v6 \t v7 \t v8 \t v9 \t v10 \t v11 \t v12 \t A \t b1 \t b2 \t b3 \t b4 \t b5 \t b6 \t b7 \t b8 \t b9 \t b10 \t b11 \t b12 \t sv \t t0'];
...
%         for j = 1:10
%             if temp_params(j) > 1 && temp_params(j) < 2  % don't use drift rate between 1 and 2
%                 LL(i,4) = NaN;  
%             end
%         end
%     end
...
opt_params_summary=NaN(54,15);
opt_LL=NaN(54,4);

for i =1:maxidnum
% for i = [5, 7, 11, 15, 17, 21, 23, 27, 33, 39, 41, 46, 47, 54]
% for i = [1, 2, 6, 8, 9, 10, 12, 13, 16, 18, 19, 22, 26, 28, 29, 30, 32, 34, 35, 36, 37, 38, 40, 42, 43, 44, 45, 48, 50, 52]
    if i == 14 || i == 24 || i == 20 || i == 49 || i == 51 || i == 53 % have overall accuracy < 70% (determined in R)
        opt_params_summary(i,:)=repelem(NaN,15);
        opt_LL(i,:)=repelem(NaN,4);
    else
        opt_params_summary(i,:)=ind_opt_params{1,i}{1,4};
        opt_LL(i,:)=ind_opt_LL{1,i};
    end
end

opt_params_summary_m3=NaN(54,16);
opt_LL=NaN(54,4);
...
% for i = [1, 2, 6, 8, 9, 10, 12, 13, 16, 18, 19, 22, 26, 28, 29, 30, 32, 34, 35, 36, 37, 38, 40, 42, 43, 44, 45, 48, 50, 52]
    if i == 14 || i == 24 || i == 20 || i == 49 || i == 51 || i == 53 % have overall accuracy < 70% (determined in R)
        opt_params_summary_m3(i,:)=repelem(NaN,16);
        opt_LL(i,:)=repelem(NaN,4);
    else
        opt_params_summary_m3(i,:)=ind_opt_params{1,i}{1,3};
        opt_LL(i,:)=ind_opt_LL{1,i};
    end
end
...
for i=1:27
    figure(i)
    histogram(opt_params_summary(:,i),'FaceColor','b')
    title(['parameter' + string(i)])
end

for i=1:15
    stats(i,1) = mean(opt_params_summary(:,i),'omitnan');
    stats(i,2) = std(opt_params_summary(:,i),'omitnan');
end


for i=1:16
    figure(i)
    histogram(opt_params_summary_m3(:,i),'FaceColor','b')
    title(['parameter' + string(i)])
end

for i=1:16
    stats(i,1) = mean(opt_params_summary_m3(:,i),'omitnan');
    stats(i,2) = std(opt_params_summary_m3(:,i),'omitnan');
end

%% Compare plots of data and model fit - not for subject level
...
for sub = 1:maxidnum
    ...
    Ncond = max(data.cond);
    cor = data.response == data.stim;
    [v A b sv t0] = LBA_parse(model(4), opt_params_summary(sub,:), Ncond);
 
    % Plot data and predictions
    LBA_plot(data, opt_params_summary(sub,:), model(4));
    title(['Subject ' + string(sub)])
...
x5 = chi2inv(0.95,5);  % critical chi-square value for alpha=.05 and 5 DOF is 11.0705
x10 = chi2inv(0.95,10);  % critical chi-square value for alpha=.05 and 10 DOF is 18.3070
x10a = chi2inv(0.99,10);  % critical chi-square value for alpha=.05 and 10 DOF is 23.2093
x11 = chi2inv(0.95,11);  % critical chi-square value for alpha=.05 and 11 DOF is 19.6751
x22 = chi2inv(0.95,22);  % critical chi-square value for alpha=.05 and 22 DOF is 33.9244
x26 = chi2inv(0.95,26);  % critical chi-square value for alpha=.05 and 22 DOF is 38.8851
...
chi_sq_1v2 = -2*mean(opt_LL(:,1),'omitnan') - (-2*mean(opt_LL(:,2),'omitnan'));  % model 2 is more general, model 1 is specific
chi_sq_1v3 = -2*mean(opt_LL(:,1),'omitnan') - (-2*mean(opt_LL(:,3),'omitnan'));  % model 3 is more general, model 1 is specific

chi_sq_2v4 = -2*mean(opt_LL(:,2),'omitnan') - (-2*mean(opt_LL(:,4),'omitnan'));  % model 4 is more general, model 2 is specific
chi_sq_3v4 = -2*mean(opt_LL(:,3),'omitnan') - (-2*mean(opt_LL(:,4),'omitnan'));  % model 4 is more general, model 3 is specific
...
for fi = 1:maxidnum
    ...
    for i = 1:length(real_data)
        ...
        elseif real_data(i,3) == 'easy' && real_data(i,4)=='high' && real_data(i,5) == 'DECIDE'
            real_data(i,end) = 5;   % decide_HEclose
